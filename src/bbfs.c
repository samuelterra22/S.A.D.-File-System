...
int bb_getattr(const char *path, struct stat *statbuf) {
	...
	if (strcmp(path, "/bug.txt") == 0) {
		log_msg("TENTANDO LER O BUG.TXT\n");
...
int bb_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
	...
	if (fi->fh == 99) {
		log_msg("PRINT HELLO WORLD\n");
		strcpy(buf, "world");
		return (int) strlen(buf) + 1;
	} else {
		log_msg("NAO EH 99: %d", log_syscall("pread", pread(fi->fh, buf, size, offset), 0));
		log_msg(buf);
